...
function Script_Variable_init(){
    Script_Version="ArchLinux User Install Scripts v4.6" 
...
usage: ${0##*/} [-h] [-V] command ...
    ...
    Install Commands: (\"-S = install\", \"-R = uninstall\"):
        ...
        fcitx   Install Fcitx input method.
        ibus    Install Ibus input method.
        axel    Install and configure Axel (Pacman multi threaded download).   
...
function Install_Font(){
    ...
    local User_options_install_fonts_Options=$2
    case ${1} in
        # 根据配置文件安装相应的字体
        "Config_file_install_fonts") Config_file_install_fonts;;
        # 根据用户选项安装相应的字体
        "User_options_install_fonts") User_options_install_fonts "$User_options_install_fonts_Options";;
        # 脚本运行时，由脚本自动判断，自动安装配置文件中的选项，另外询问是否安装其他
...
function Auins_Options(){
    ...
    # Global Options:
        -e | --edit-conf  ) vim "${Auins_Config}"; exit 0 ;;
        -f | --view-conf  ) less "${Auins_Config}"; exit 0 ;;
        -i | --view-info  ) clear; less "${Auins_record}"; exit 0 ;;

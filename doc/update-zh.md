## Auins 脚本更新日志
- [**2022.xx.xx**]  v4.3.5更新说明 新增与改进：
	1. 此版本更新后，将进入v5.0，再做自动化安装配置：硬件，桌面等，以及无人值守;
	2. 优化：`auin.sh`;
	3. 新增：`install.conf` 部分接口;
	4. 内核：新增选择 `Linux-zen`;
	5. 脚本：去除 `Edit_Database.sh`;
	6. 脚本：更新  `Process_manage.sh` `Useradd.sh` `Partition.sh`;
	7. 脚本：[v5.0]测试 `s2arch.sh` 可在VPS上安装Archlinux;
	8. 脚本：新增启动脚本时，先打印计算机信息，如：CPU架构，LiveCD，环境，网络，磁盘等;
	9. 脚本：重写镜像源的配置脚本;
...
- [**2020.12.16**]  v4.0.5 更新说明 新增与改进：
    ...
    5. 去除模块: `Module/setting_xinitrc.sh` 整合至`auin.sh`;
